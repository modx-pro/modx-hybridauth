...
class Odnoklassniki extends OAuth2
{
    ...
    public function getUserProfile()
    {
        ...
        $userProfile->profileURL = 'https://ok.ru/profile/' . $data->get('uid');

        // OK API returns YYYY-MM-DD.
        if ($data->get('birthday')) {
            list($birthYear, $birthMonth, $birthDay) = explode('-', $data->get('birthday'));
            $userProfile->birthDay = (int)$birthDay;
            $userProfile->birthMonth = (int)$birthMonth;
            $userProfile->birthYear = (int)$birthYear;
        }
